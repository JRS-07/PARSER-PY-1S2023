package jflex;
import static modelo.TipoToken.*;
import estrada.Token;
...
while { 
    ...
    return new Token(PALABRA_RESERVADA, yytext(), lineaActual, tokenColumna); }
...
True|False { 
    int tokenColumna = columnaActual;
    columnaActual += yytext().length();
    return new Token(BOOLEAN, yytext(), lineaActual, tokenColumna); }
...
"(" | ")" | "{" | "}" | "[" | "]" | "," | ";" | ":" { 
    int tokenColumna = columnaActual;
    columnaActual += yytext().length();
    return new Token(DELIMITADORES, yytext(), lineaActual, tokenColumna); }
...
""="" | "+=" | "-=" | "*=" | "/="  { 
    int tokenColumna = columnaActual;
    columnaActual += yytext().length();
    return new Token(ASIGNCION, yytext(), lineaActual, tokenColumna); }
